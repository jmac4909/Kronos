...
<p align="center">
  ...
  <img alt="438 automated local tests" src="https://img.shields.io/badge/test%20suite-438%20local-22C55E">
  <img alt="Preview status" src="https://img.shields.io/badge/status-preview-F59E0B">
</p>

> **Preview software.** Kronos is ready for local evaluation, not represented as a Marketplace release or production service. Source is available under the included [source-review license](LICENSE).
...
| --- | --- |
| [Product contract](docs/terminal-first-product-contract.md) | Two explicit terminal-write paths, with submission owned by the operator |
| [State ownership](docs/state-ownership.md) | How sessions, provider evidence and private artifacts keep distinct owners and lifecycles |
| [Provider contracts](docs/provider-contract-matrix.md) | Bounded read-only integrations, freshness, incomplete responses and credential boundaries |
| [Verification matrix](docs/verification-matrix.json) | Executable evidence linked to features, with real-editor, Windows and live-provider checks left explicit |

The runtime uses only VS Code and Node built-ins. Examples use synthetic data; the verification matrix distinguishes automated checks from pending real-editor and live-provider validation.
...
| **Sessions** | Organize operator-owned Claude terminals by local project, give any Session a presentation-only display name, launch them as main-editor tabs or separate side-by-side editor columns, balance visible columns, maximize or restore one explicitly, attach multiple Jira contexts when useful, and never read terminal contents. |
...
| **Attention** | Act as a quiet project delivery monitor: show a project only while an MR is open, a build is running, status is incomplete, a check needs fixes, or review/merge is ready. Current GitLab, Jenkins, and SonarQube rows stay green while healthy, failures and incomplete reads remain visible until current truth recovers, and completed healthy work disappears. Completed Jenkins results and available SonarQube gates can be cleared locally with an inline X until their current fingerprint changes; running and pending checks stay visible. Provider pages and fresh MR/CI context remain one click away; exact retained events remain available when a current row has matching audit evidence. |
...
9. Decide whether to press Enter yourself; project-level **Attention** then tracks active delivery work and tells you when checks need fixes, status is incomplete, or an MR is ready for review or merge.
...
The second render is an explicitly labeled interaction sequence: the composer is shown before insertion, and the terminal panel shows the state after the composer closes. Both renders use synthetic `DEMO-*` records and example provider state.
...
| --- | --- | --- |
| Explicit Claude start/resume | Create and focus a new terminal after **New Claude** from Sessions/a Project or **Start Claude for Ticket**; resume a disconnected managed Session only when selected | Accepts only a validated `claude` or `claude-*` executable, narrowly allowlisted interactive flags, one typed permission mode, and a Kronos-generated exact Claude session UUID; optional confirmed lifecycle tracking keeps that UUID and its resume folder current after Claude switches conversations; experimental bypass requires a modal confirmation every time |
...
Every newly managed Claude terminal is started with a Kronos-generated Claude session UUID and non-secret VS Code terminal identity markers. Reload Window reconnects the same terminal object without another command. After a full VS Code or machine restart, the Session shows **Resume available** and selecting it explicitly runs `claude --resume <exact UUID>` in the saved folder, reusing the marker-matched revived terminal when present. Setup can optionally install one confirmed user-level `resume|clear` `SessionStart` hook so `/resume`, `/clear`, and a later `claude --resume` in the same managed terminal update only that binding's current UUID and saved resume folder. Ordinary startup and compaction do not invoke the helper. It receives bounded lifecycle metadata, emits no stdout context, writes one correlated private event, never opens `transcript_path`, and preserves project, Jira, provider, display-name, and terminal ownership. Sessions created before managed identity existed offer Claude's own conversation picker; Kronos never guesses from terminal text, name, process ID, or current directory. No tmux layer is required.
...
The installed extension uses the VS Code API and Node built-ins only. It has **zero third-party runtime dependencies** and does not bundle an agent SDK or provider client library. Optional Claude lifecycle tracking writes one audited local Node helper only after operator confirmation; that helper runs solely for `resume|clear` SessionStart events and emits no Claude context.
...
| Manifest-covered commands | 45 |
| Manifest-covered settings | 14 |
| Reachable runtime modules checked for cycles/dead exports | 100 |
| Third-party runtime dependencies | 0 |
| Automated Node/DOM/board tests | 438 |
| Built-in runtime coverage floor | ≥95% lines / ≥95% branches / ≥95% functions |
...
Project integration expects a reusable Jenkins job or multibranch-parent page ending in `/job/<name>/`; nested folders repeat `/job/`. Do not enter a Jenkins server root, `/branch/`, view, console, REST API, or numbered-build URL. The explicitly saved SonarQube project and branch remain the monitoring target even when the local repository is checked out to another branch that has not been analyzed.
...
`npm test` begins with a public-surface gate that rejects tracked local-state directories, machine-specific home paths, configured private markers, private-key material, and high-confidence token shapes. The remaining checks enforce the manifest allowlist, a reachable runtime graph restricted to local modules, Node built-ins, and the VS Code API, the explicit-launch boundary, context governance, strict TypeScript compilation, unit behavior, cross-view product contracts, provider fixtures, large synthetic scale/accessibility behavior, exact dependency-free VSIX file surface, dependency-free webviews, and Node's built-in line/function/branch coverage floors. The coverage pass derives its 33-runner inventory from the actual `npm test` graph, enforces at least 95% lines, branches, and functions, fails closed when a runner or report row disappears, and retains tighter per-file regression floors for the shared bounded transport, all four provider REST clients, GitLab evidence normalization, CI snapshot persistence, the active Attention monitor, exact Attention-event snapshots, Jira normalization/pruning, Work state/presentation, exact terminal insertion and Claude launch, project and provider monitoring, Context Basket, local handoff, local evidence search, date normalization, GitLab pipeline transition normalization, the monitor-event ledger and audit view, webview CSP/bootstrap security, provider-read health, Attention, Sessions, and Projects. `npm run package` repeats the public, manifest, graph, security, governance, roadmap, claim, compile, and exact-surface gates before creating a VSIX; extra or missing files fail closed. The checked [verification matrix](docs/verification-matrix.json) maps roadmap goals to named tests and keeps real VS Code, operator-terminal, Windows, multi-window, and live-provider gates explicit. README engineering metrics are derived from the manifest, source tree, dependencies, and test declarations on every run.

Contributor setup and repository-safety procedures are in [CONTRIBUTING.md](CONTRIBUTING.md#repository-safety).

The [CI workflow](.github/workflows/ci.yml) runs the same `npm test` gate for pushes and pull requests using synthetic fixtures, without provider credentials. CI is not a substitute for real-editor acceptance.
...
src/services/claudeTerminalLauncher.ts   explicit validated Claude start/resume
src/services/managedClaudeTerminalIdentity.ts durable resume identity validation
src/services/claudeSessionTracking.ts   optional confirmed lifecycle reconciliation
