...
Kronos may place a newly and explicitly launched Claude terminal in the VS Code editor area, create a separate editor column beside the active group for each later launch, or place it in the terminal panel according to the visible layout setting. The operator may use the Sessions layout control to change future launch placement or explicitly balance the visible editor columns. Kronos may also focus an explicitly selected managed terminal and ask VS Code to maximize or restore that terminal's editor group or panel. These presentation actions never read terminal content, move pre-existing terminals automatically, submit input, interrupt a process, or close a terminal.
...
- creates a terminal except after **New Claude**, **Start Claude for Ticket**, or explicit selection of a disconnected Session that needs a fresh resume terminal;
...
An explicit Claude-start or resume action validates its configured executable, approved interactive flags, typed permission mode, terminal name, and working directory before creating or writing anything. Positional prompts/subcommands and raw permission, session-management, tool-allowing, directory-expanding, MCP, plugin, background, and non-interactive flags are rejected. The typed launch mode is one of Manual/default, Accept Edits, Plan, Auto, Don't Ask, or Bypass Permissions. Non-default modes are appended only by the launcher. Bypass Permissions is explicitly experimental: Kronos shows a blocking warning for every requested bypass start or resume, offers launch, Claude Settings, or cancel, and creates no session or terminal unless the operator chooses the launch action. A new managed terminal receives a generated UUID through `--session-id`; an exact resume receives only the matching persisted UUID through `--resume`. A legacy Session may explicitly open Claude's own bare `--resume` picker. The launcher creates or reuses and focuses one validated VS Code terminal and executes the resulting command exactly once. It does not use a subprocess library, observe whether Claude succeeded, or read the resulting terminal.
...
Persisted terminal names and process IDs are descriptive metadata, not durable identity. New managed Claude bindings instead persist a current Claude UUID, the UUID fixed in that terminal's launch environment, the VS Code editor session identifier last attached, and the typed permission mode. Their terminals carry non-secret work-session, terminal-binding, launch-time Claude-session, and editor-session environment markers. Markers are candidate identity only: Kronos re-resolves the private Session record and requires the exact work-session and binding plus either its current or correlated launch-time Claude UUID before use. During Reload Window in the same VS Code editor session, Kronos restores that live object association without submitting another command. After a full editor or machine restart changes the editor session, Kronos does not launch automatically; selecting the disconnected Session explicitly resumes the current exact UUID in its saved folder, reusing a correlated marker-matched revived terminal when available. A legacy Session without a UUID offers Claude's own conversation picker and explicitly listed unclaimed terminals. Kronos never infers a conversation from terminal contents, name, process ID, or folder. Context insertion remains blocked until a live object association exists.
...
Supported Session actions are Open Terminal, Connect Focused Terminal, Disconnect Terminal, Rename Session, Pause Updates, Resume Updates, Check Updates, View History, Stop Tracking, and confirmed Remove from Kronos. **Rename Session** changes only an optional local display name; the generated title, Session ID, terminal, exact Claude resume identity, project, Jira contexts, provider bindings, and audit ownership remain unchanged. Blank input restores the generated title. Session rows keep only connection and provider-health state visible; project, branch, Jira keys, timestamps, and saved-context details remain available in the tooltip. A stopped row says **Tracking stopped** rather than implying that Kronos closed the operator-owned terminal. Removal never closes a terminal; it removes the session record and colocated monitor snapshots while retaining shared history and saved context.

Selecting any Session means “open its terminal.” A live attachment is focused immediately. A same-editor Reload Window reconnects a matching managed terminal without sending a command. After a full editor or machine restart, an identified Session shows **Resume available** and selection explicitly resumes its exact Claude UUID. When more than one durable conversation or marker-matched terminal is available, Kronos requires a choice. Legacy Sessions require an explicit choice between Claude's conversation picker and listed unclaimed terminals. Kronos never guesses from a saved process ID, duplicate terminal name, terminal contents, or folder.

Setup and Check Setup include optional **Claude Session tracking**. Enabling or repairing it always requires modal operator confirmation. Kronos then creates a private local helper, keeps a private backup of an existing user Claude settings file, and merges one identifiable `SessionStart` command hook without replacing unrelated settings or hooks. The hook is matched only for `resume|clear`; ordinary startup and compaction do not spawn the helper because neither changes the managed conversation identity. Invalid, oversized, symbolic-link, or unsupported settings fail closed. The helper no-ops outside a Kronos-managed terminal, emits no stdout context, and consumes only bounded lifecycle input (`session_id`, `cwd`, event name, and source); it never reads the supplied transcript path. A private event is accepted only when its work-session ID, terminal-binding ID, and fixed launch UUID match an active managed binding and its timestamp is newer. Reconciliation may change only that binding's current Claude UUID, saved resume folder, a missing fixed-launch correlation, and observed timestamp. Project, Jira contexts, providers, Session display name, permission mode, editor identity, and terminal ownership remain unchanged. This supports Claude's `/resume` and `/clear` plus a later `cd ...; claude --resume ...` launched from the same managed shell without inspecting shell input or terminal content.

**New Claude** creates a project-oriented session, validates the configured Claude command/permission-mode/name/cwd, generates a fresh Claude UUID, creates and focuses one terminal, and starts Claude with that exact session identity. From Sessions it uses the configured workspace/home launch choice; from a registered Project it always uses that exact project directory. The created terminal tab includes the branch read from the actual launch directory when Git `HEAD` is available. **Start Claude for Ticket** also includes the initiating ticket key in that title. This is launch-time display metadata only: Kronos does not invoke Git and does not write to or rename an existing terminal. A new project session contains no fake or placeholder ticket key. An operator may later attach one or more real Jira contexts to any explicitly managed terminal; this never creates or submits terminal input automatically. Legacy ticket-keyed records remain readable for migration compatibility, but Sessions and monitoring are presented and deduplicated by project when a project is known.
...
Session lifecycle has three independent axes: management is active or stopped; the terminal relationship is none, attached, detached, or closed; and monitoring is running, paused, ineligible, or stopped. Only a live in-memory VS Code Terminal binding is attached. A same-editor Reload Window may restore an exact managed binding from matching creation metadata without a terminal write. A full editor or machine restart leaves it detached until explicit exact resume. A terminal close event records closed terminal history while leaving the Session available for a new explicit resume or reconnect. Stopping management records stopped management and detached terminal metadata because Kronos did not close the operator-owned terminal.
...
Project setup accepts a reusable Jenkins job or multibranch-parent page whose path ends in `/job/<name>/`; nested folders repeat `/job/`. The server root, `/branch/`, `/view/`, `/console`, REST API, and numbered-build URLs are not project monitoring targets. A multibranch parent is paired with the default monitoring branch or an exact branch-routing profile so Kronos can resolve the child job without storing a fixed build.

Project setup may store at most 20 explicit branch-routing profiles. Each exact match branch can override the Jenkins job URL, SonarQube project key, and SonarQube provider branch for read-only evidence; one configured profile may be the fallback. An exact observed local or linked-MR source branch wins before that fallback. Branch names and provider identifiers are validated, credential-bearing URLs are rejected, and malformed persisted profiles are omitted at Work-catalog ingress. Profiles belong only to an explicitly registered project. Saving valid provider setup activates bounded project-owned polling immediately. The current local branch may select an exact routing profile and guide GitLab MR discovery, but it does not replace an explicit default SonarQube monitoring branch; it is only the SonarQube fallback when no provider branch is configured. A profile becomes ticket-visible only through an explicit ticket-project link. Profiles never infer a link from a Jira namespace, select or switch a Git branch, change a worktree, or write to a provider.
...
Attention is the project-owned active-delivery monitor. Projects remains the complete registered repository inventory; Attention derives current GitLab, Jenkins, and SonarQube state for every registered project and renders only projects with active delivery work. A project is visible while it has an open merge request, an active GitLab pipeline or Jenkins build, an unresolved provider/check failure, incomplete current status, or a review/merge milestone. A project with no open MR, no running build, no unresolved problem, and complete healthy checks is omitted. Historical transitions remain in the private append-only audit and never keep an otherwise finished project visible.

Each visible project has one overall state: **Needs fixes**, **Status incomplete**, **Ready for review**, **Ready to merge**, or **In progress**. Needs-fixes and incomplete projects sort first, followed by review-ready, merge-ready, and healthy in-progress work. Current healthy/running rows use the shared green state color; partial or review-needed state is yellow; failure and blocking state is red. Text labels carry the same meaning without requiring color. The group shows the last live check time, while row tooltips retain the current subject, observed time, changed time, and reason.

Review readiness is conservative. **Ready for review** requires an open MR, a successful GitLab pipeline, every configured Jenkins and SonarQube check present and passing, complete approval evidence, and approval still required. When those same requirements are satisfied and approval requirements are complete, the project becomes **Ready to merge**. Missing configured evidence stays **In progress** or **Status incomplete**; it never produces a false-ready state. Provider-read failure or partial state replaces stale current truth until the current read status recovers. Read health is scoped to the exact current provider resource: a retained SonarQube branch's partial or failed read remains audit history and never overrides another branch that is now configured and healthy.

Visible rows come from the latest normalized monitor snapshots. When a retained transition has the exact current fingerprint, Attention reuses that event and may expose **Use Event in Prompt** for eligible GitLab MR, Jenkins, or SonarQube evidence. Otherwise it renders a deterministic read-only live row and offers fresh project MR/CI context instead of pretending that an exact transition exists. A Jenkins build may be cleared locally only after it stops running, and a SonarQube row only after its quality gate becomes available. The inline X appends a local acknowledgement for that exact current event/fingerprint without changing provider truth or deleting audit history; the row returns when the provider result changes. Running Jenkins builds, pending/unavailable SonarQube gates, provider-read failures, GitLab rows, and monitoring blockers remain non-dismissible. Legacy or unassigned transition rows remain a compatibility fallback and may still be cleared locally without changing provider state.

Pop-up notifications are deliberately narrower than the monitor. Kronos warns for newly observed failures, blocked monitoring, and incomplete provider reads. It announces **Ready for review** once when a transition completes all configured checks while approval is still needed. Ordinary start/running/success/recovery state, healthy in-progress work, and **Ready to merge** remain visible in Attention without notification noise. A later non-ready state resets the review-ready announcement boundary so a future genuine return to review-ready may announce again.

Applicable rows may open a validated provider URL, open the correctly scoped editable composer for fresh MR/CI context, freeze one eligible exact retained transition, or open audit history. These actions use the registered project's canonical name/path and an explicitly attached project Session; they never require or fabricate a Jira ticket. If multiple retained SonarQube branches or Jenkins builds are available, opening the provider uses a native latest-first picker. Choosing a different SonarQube branch updates only the local monitoring target, opens the provider page, and starts a fresh immutable CI review when possible; older prompt files remain audit history. A missing Session, project association, registered project, URL parameter, or refreshable project folder produces a visible bounded warning and output entry instead of a silent return. SonarQube dashboard URLs retain only non-secret `id` and `branch` routing parameters.

## Context Insertion Contract

Context insertion is always explicit and terminal-scoped. Jira evidence remains ticket-scoped. MR and CI evidence may be scoped either to an explicit ticket or directly to a registered project, and the operator may choose any active explicitly managed terminal. An `[ATTENTION-*]` snapshot contains exactly one eligible retained transition and may use the explicit project or ticket owner already carried by that event. A `[GIT-project]` working-tree snapshot is project-scoped and may be inserted into an explicitly attached session for that project. A `[PROMPT-*]` team-prompt snapshot may be opened from Work, Sessions, Projects, or a ticket workspace, but it still requires an active explicitly managed terminal. Creating a Claude session does not silently create ticket context or a ticket association.

1. Kronos resolves the selected ticket and the explicitly managed terminal.
2. For fresh provider context, it reads the configured provider through bounded read-only APIs. For exact Attention-event context, it rereads only the selected normalized private ledger event and performs no provider request.
...
- `[ATTENTION-SONAR-*]`: exactly one retained GitLab MR, Jenkins, or SonarQube transition, including its bounded subject, before/after state, metadata, severity, and observed time; no broader provider state is implied.
...
- Sessions: create a project-oriented Claude session; choose side-by-side editor columns, editor tabs, or the terminal panel for future launches; explicitly balance visible editor columns; add another Jira context; set or clear a presentation-only Session display name; open the Team Prompt Library; check provider updates; search local session and saved-context metadata; create a local handoff; view history; open, connect, or disconnect a terminal; stop tracking or remove a stopped Session; pause or resume updates;
...
- Attention: monitor current active project delivery, use one supported exact retained event in a prompt when available, acknowledge legacy fallback items, and open providers or fresh project context;
...
Visible view-title actions follow one checked hierarchy. Work shows Refresh Jira, Jira Board, and Filter. Sessions shows New Claude, Connect Focused Terminal, and Terminal Layout. Projects shows Refresh Projects and Manage Projects. Attention shows Check Updates. Clear Filters, Context Basket, Search, Handoff, and secondary update actions remain in the relevant view overflow menus. Setup is available from Work overflow and the command palette; Check Setup and advanced settings are reached from Setup or the command palette rather than repeated in every view header. Work rows show **Start Claude**, Sessions show **Open Terminal** or **Connect Focused Terminal**, Projects show **Start Claude**, and Attention shows **Open Provider** as the routine inline action. A completed Jenkins result or available SonarQube gate may additionally show the inline **Clear from Attention** X defined by the local-acknowledgement contract; no other live row gains a second inline action. Ticket-row right-click menus contain only ticket-scoped actions; terminal connection and the general Team Prompt Library stay in their owning global surfaces. Project-row menus similarly omit the general prompt-library shortcut because it remains in the Projects overflow. Sessions offer Rename Session without changing durable identity; active Sessions also offer Disconnect Terminal, Pause or Resume Updates, and Stop Tracking, while Remove from Kronos appears only after tracking has stopped. Exact retained Attention rows put **Use Event in Prompt** first and keep broader fresh MR/CI review separate. Clearable current CI rows and legacy fallback rows expose **Clear from Attention** only as local presentation state; running/pending CI, GitLab, provider-read, and monitoring-blocker rows remain non-dismissible. All menu surfaces use concise action labels while the command palette retains the `Kronos:` prefix for search and disambiguation. Work filters use plain labels such as **Search**, **Work state**, and **Clear all filters**, and show the current selection beside each choice.
...
8. Active project delivery appears in Attention until the MR/build completes or current failures resolve, with exact-event or fresh MR/CI context actions as evidence allows.
