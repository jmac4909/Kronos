...
**Goal statement:** A poll shall produce an audit transition only when normalized provider state meaningfully changes, while Attention derives the current active-delivery state for each registered project and the append-only audit retains complete history.

Cases to cover:

- An open MR remains visible from current snapshots even when initially healthy and mergeable.
...
- Live project rows cannot be cleared while their current state remains active.
- Merged or closed MRs disappear after current builds and unresolved problems complete.
- Newer pipelines, jobs, builds, test results, quality gates, and Sonar issues replace stale rows.
- Historical acknowledgements never change live project truth or resurrect an older row.
...
- The audit can reconstruct the sequence even when Attention contains only current snapshot rows.
...
- An optional display name can be set or cleared without changing the generated title, Session ID, terminal, Claude resume identity, project, Jira contexts, or provider bindings.
...
**Goal statement:** Attention shall quietly monitor active project delivery work, showing healthy progress without notification noise and clearly identifying failures, incomplete status, review readiness, and merge readiness.

Cases to cover:

- MR, pipeline, Jenkins, SonarQube, provider health, and local monitoring blockers.
- Open MR, running build, healthy progress, failure, incomplete status, ready-for-review, and ready-to-merge states.
...
- Live current state cannot be dismissed; historical audit and legacy fallback clearing remain understandable.
- Notifications occur only for problems and the first transition into ready-for-review.

Completion evidence:

- Rows visibly show project, overall delivery state, provider, subject, last checked time, last changed time, and why the work remains active.
