...
| Attention monitor and ledger | One current projection for each of 200 registered projects plus 2,000 newest fallback ledger rows; 50 MiB ledger read ceiling and 16 KiB per event | Only projects with an open MR, active build, unresolved failure, incomplete status, or readiness milestone render; finished healthy projects disappear and history remains in the audit. |
...
- Render a 2,000-item Session history into its exact newest 500-item Markdown summary in under 1,000 ms and under 2 MiB without mutating the source.
