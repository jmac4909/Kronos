...
- Bug 3 is validated on the Windows EC2 lab with VS Code 1.127.0. Jira Board, Run Center, Evidence Gate, and Human Review Inbox opened as webviews, reported script/action readiness, accepted click actions, and produced zero suspicious CSP/runtime logs in the CDP smoke.
...
- Feature 13 is implemented with periodic review MR polling. Open review MRs update persisted MR state/review/comment metadata, merged MRs transition the ticket to `deploy_monitor`, and Kronos starts a deploy-monitor run when no active or completed deploy monitor already covers that ticket/MR/project. Failed preflight handoffs remain retryable and are recorded as ticket evidence instead of being treated as handled.
...
- Windows validation note: the lab is Windows Server rather than Windows 11, but it is running VS Code 1.127.0 with the packaged `jmacke01.kronos@0.1.0` VSIX. The latest CDP smoke used `Code/1.127.0`, Chrome `148.0.7778.97`, and Electron `42.2.0`; all four previously dead webviews returned `ready=true` and `actionsReady=true`, click checks succeeded, and suspicious logs were `0`. Host artifact directory: `/home/ec2-user/projects/codex-aws-lab/artifacts/kronos/20260703T160939Z-26495e5-current-vsix-cdp-smoke`.
