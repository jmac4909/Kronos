...
test('project integration values round-trip, clear, and default to the observed Git branch', () => {
  ...
  for (const invalidJenkinsUrl of [
    'https://jenkins.example/',
    'https://jenkins.example/branch/application/',
    'https://jenkins.example/view/team/job/application/',
    'https://jenkins.example/job/application/42/',
    'https://jenkins.example/job/application/api/json',
  ]) {
    assert.throws(
      () => projectCatalog.setLocalProjectIntegrations(initial, [{
        name: 'Application',
        jenkinsUrl: invalidJenkinsUrl,
      }]),
      /full HTTPS Jenkins \/job\/<name>\/ URL.*without.*numbered build/i,
      `project setup rejects non-reusable Jenkins target ${invalidJenkinsUrl}`,
    );
  }
  ...
  assert.match(html, /Jenkins job URL \(or multibranch parent\)/);
  assert.match(html, /Expected \/job\/ path/);
  assert.match(html, /Nested folders repeat \/job\//);
  assert.match(html, /Default monitoring branch or Branch routing selects the branch/);
  assert.match(html, /do not use \/branch\//i);
  assert.match(html, /\/view\/, \/console, \/api\/json/i);
  assert.match(html, /numbered build URL/i);
  assert.match(html, /value="team:application"/);
  assert.match(html, /value="feature\/observed-branch"/);
  assert.match(html, /merely checking out another local branch does not/i);
...
test('project catalog edge matrix validates identity, integration, profile, and Git pointer boundaries', t => {
  ...
  }]), /full HTTPS Jenkins \/job\/<name>\/ URL/i);
  assert.throws(() => projectCatalog.setLocalProjectIntegrations(initial, [{
