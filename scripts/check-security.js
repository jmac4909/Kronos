...
const serviceOwnedSafetyMarkers = new Set([
  ...
  'export function isStaleFinishedArchiveRun',
...
for (const marker of [
  ...
  'extractTicketAcceptanceCriteria',
  ...
  'function pollReviewMergeRequests(state: KronosState, shouldContinue: () => boolean = () => true): Promise<ReviewPollResult>',
...
for (const marker of [
  ...
  "const notificationKey = notification ? notification.message : 'notify'",
  'update.mergedNow',
  "kind: 'deploy_monitor'",
  'update.closedNow',
  "kind: 'blocked'",
...
for (const [name, source, marker] of [
  ['src/extension.ts', extension, "import { formatDateTimeLabel, formatTimeLabel } from './services/dateLabels'"],
...
for (const [name, source, marker] of [
  ...
  ['src/services/ticketMutations.ts', ticketMutations, "import { isRecord, optionalTrimmedStringFromUnknown, recordsFromUnknown } from './records'"],
...
for (const [name, source, marker] of [
  ...
  ['src/services/postRunReadiness.ts', postRunReadiness, "import { arrayFromUnknown, optionalFiniteNumberFromUnknown, optionalTrimmedStringFromUnknown, recordFromUnknown } from './records'"],
]) {
  if (!source.includes(marker)) {
