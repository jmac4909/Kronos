...
function checkClaudeLauncherContract() {
  ...
  if (!/const\s+configuration\s*=\s*normalizeClaudeTerminalLaunch\(input\)[\s\S]{0,240}submitClaudeTerminalCommand\(factory,\s*configuration,\s*presentation\)/.test(code)
    || !/const\s+configuration\s*=\s*normalizeClaudeTerminalResume\(input\)[\s\S]{0,240}submitClaudeTerminalCommand\(factory,\s*configuration,\s*options,\s*options\.existingTerminal\)/.test(code)
    || !/function\s+submitClaudeTerminalCommand\([\s\S]{0,1800}factory\.createTerminal\(terminalOptions\)/.test(code)) {
