...
function checkReadmeMetrics(packageJson, readmeSource) {
  ...
  const coverageFloor = '| Built-in runtime coverage floor | ≥95% lines / ≥95% branches / ≥95% functions |';
  if (!readmeSource.includes(coverageFloor)) {
    fail('README engineering proof must state the stable 95% runtime coverage floors instead of copied run-specific percentages.');
  }
}
