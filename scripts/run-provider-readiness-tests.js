...
const tempRoot = fs.realpathSync.native(fs.mkdtempSync(path.join(os.tmpdir(), 'kronos-provider-readiness-')));
...
test('one readiness snapshot feeds Setup and Doctor and gives every non-ready row an action', () => {
  ...
  const snapshot = operationsReadiness.buildOperationsReadiness({
    ...
    claudeSessionTracking: {
      status: 'warn',
      detail: 'Optional tracking is not enabled.',
  ...
  const liveFailureSnapshot = operationsReadiness.buildOperationsReadiness({
    ...
    claudeSessionTracking: {
      status: 'pass',
      detail: 'Claude Session tracking is enabled.',
