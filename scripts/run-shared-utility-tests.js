...
test('project command presentation plans discovery, removal, integration forms, and ticket choices', () => {
    ...
    projects: {
      app: {
        ...
        config: {
          ...
          default_branch: 'main',
          branch_profiles: [{ branch: 'release', jenkins_url: 'https://jenkins/job/app/job/release' }],
  ...
  assert.deepEqual(forms[0], {
    ...
    branchProfiles: 'release | https://jenkins/job/app/job/release |  | ', activeBranchProfile: 'release',
  });
...
test('operations readiness covers provider environment, project, catalog, diagnostic, and plural states', () => {
  const base = {
    ...
    claudeSessionTracking: { status: 'warn', detail: 'Optional tracking is not enabled', actionLabel: 'Enable Tracking' },
  ...
  assert.equal(empty.find(item => item.id === 'claude-session-tracking').action, 'configureClaudeSessionTracking');
  ...
  const environmentDetails = [
    [{ present: false, invalid: 0, configuredProviders: 1, path: '/private/providers.env' }, /1 provider available/],
    [{ present: false, invalid: 0, configuredProviders: 2, path: '/private/providers.env' }, /2 providers available/],
    [{ present: true, invalid: 1, configuredProviders: 0, path: '/private/providers.env' }, /1 config entry needs attention/],
    [{ present: true, invalid: 2, configuredProviders: 0, path: '/private/providers.env' }, /2 config entries need attention/],
  ];
  for (const [providerEnvironment, expected] of environmentDetails) {
    const snapshot = operationsReadiness.buildOperationsReadiness({ ...base, providerEnvironment });
    assert.match(snapshot.find(item => item.id === 'provider-environment').detail, expected);
  }
});
