...
test('row context menus contain only actions scoped to the selected row', () => {
  ...
  assert.equal(commandsById.get('kronos.renameWorkSession').shortTitle, 'Rename Session');
  ...
  const renameSessionMenu = itemMenus.find(item => item.command === 'kronos.renameWorkSession');
  assert.match(renameSessionMenu.when, /work_session_closed/);
  assert.match(renameSessionMenu.when, /standalone_session_attached/);
  assert.doesNotMatch(
  ...
  const attentionInline = itemMenus
    ...
    .sort((left, right) => left.group.localeCompare(right.group));
  assert.deepEqual(attentionInline.map(item => item.command), [
    'kronos.openProvider',
    'kronos.acknowledgeAttention',
  ]);
  assert.equal(attentionInline[0].group, 'inline@1');
  assert.equal(attentionInline[1].group, 'inline@2');
  assert.match(attentionInline[1].when, /live_.*_clearable/);
  const eventContextMenu = itemMenus.find(item => item.command === 'kronos.insertAttentionEventContext');
  assert.equal(eventContextMenu.when, 'viewItem =~ /^attention_(provider|repair|live_provider|live_repair).*_event$/');
  assert.equal(eventContextMenu.group, 'context@1');
  const clearAttentionMenu = itemMenus.find(item =>
    item.command === 'kronos.acknowledgeAttention' && item.group === 'management@1'
  );
  assert.equal(clearAttentionMenu.group, 'management@1');
  assert.match(clearAttentionMenu.when, /live_.*_clearable/);
});
