...
import {
  ...
  type AttentionLiveProjectResult,
} from './views/AttentionTreeProvider';
import {
...
import {
  ...
  managedClaudeBindingAcceptsLaunchSession,
  readWorkSession,
  recordWorkSessionContextArtifact,
  removeWorkSession,
  reopenWorkSession,
  resumableWorkSessionTerminalBindings,
  setWorkSessionDisplayName,
  setWorkSessionMonitoring,
  setWorkSessionProject,
  updateWorkSessionClaudeIdentity,
  type ResumableWorkSessionTerminalBinding,
  type TicketWorkSessionRecord,
  type WorkSessionClaudePermissionMode,
  type WorkSessionRecord,
  workSessionEventContext,
  workSessionDisplayName,
...
import {
  ...
  acknowledgePresentedMonitorEvent,
  appendMonitorEvent,
  listMonitorEvents,
  readMonitorEvent,
...
import {
  CLAUDE_SESSION_TRACKING_INTERVAL_MS,
  claudeSessionTrackingStatus,
  installClaudeSessionTracking,
  readClaudeSessionTrackingEvent,
} from './services/claudeSessionTracking';
import {
  managedClaudeTerminalIdentity,
  normalizeEditorSessionId,
  type ManagedClaudeTerminalIdentity,
} from './services/managedClaudeTerminalIdentity';
import {
...
} from './services/gitlabMergeRequestMonitorStore';
import { readGitLabPipelineMonitorSnapshot } from './services/gitlabPipelineMonitorStore';
import { readCiMonitorSnapshot } from './services/ciMonitorStore';
import {
  activeAttentionNotificationKind,
  activeAttentionProject,
  type ActiveAttentionRow,
...
const OPERATIONS_PANEL_ACTIONS = new Set([
  ...
  'openSettings',
  'openClaudeSettings',
  'configureClaudeSessionTracking',
...
interface ManagedClaudeAttachment {
  bindingId: string;
  claudeSessionId: string;
  claudeLaunchSessionId: string;
  editorSessionId: string;
  claudePermissionMode: WorkSessionClaudePermissionMode;
  cwd?: string;
}

type LegacySessionOpenChoice =
...
class TerminalFirstRuntime implements vscode.Disposable {
  ...
  private readonly attentionTree = new AttentionTreeProvider({
    ...
    loadActiveProjects: () => this.loadActiveAttentionProjects(),
  });
  ...
  private readonly claudeResumesInFlight = new Set<string>();
  private readonly announcedReadyForReview = new Set<string>();
  ...
  private claudeSessionTrackingTimer: NodeJS.Timeout | undefined;
  private claudeSessionTrackingSuggestionShown = false;
  private readonly claudeSessionTrackingReadFailures = new Set<string>();
  ...
  constructor(private readonly context: vscode.ExtensionContext) {
    ...
    this.disposables.push(
      ...
      vscode.window.onDidOpenTerminal(terminal => this.restoreManagedClaudeTerminal(terminal)),
    ...
    for (const terminal of vscode.window.terminals) {
      this.restoreManagedClaudeTerminal(terminal);
    }
    this.reconcileClaudeSessionTracking();
    this.startTimers();
  ...
  dispose(): void {
    ...
    if (this.claudeSessionTrackingTimer) { clearInterval(this.claudeSessionTrackingTimer); }
  ...
  private registerCommands(): void {
    this.disposables.push(...registerTerminalFirstCommands({
      ...
      sessions: {
        ...
        renameWorkSession: async argument => this.renameManagedSession(argument),
  ...
  private startTimers(): void {
    ...
    this.claudeSessionTrackingTimer = setInterval(
      () => this.reconcileClaudeSessionTracking(),
      CLAUDE_SESSION_TRACKING_INTERVAL_MS,
    );
    this.refreshTimer.unref();
    this.providerTimer.unref();
    this.claudeSessionTrackingTimer.unref();
  }
  ...
  private loadActiveAttentionProjects(): AttentionLiveProjectResult[] {
    const events = listMonitorEvents({
      types: ['provider.transition', 'notification.acknowledged'],
      limit: 2000,
    });
    return listLocalProjects(this.state.state).map(project => {
      const owner = this.readProjectMonitor(project.name);
      if (!owner) {
        return { projectName: project.name, projectPath: project.path, project: null };
      }
      const mergeRequest = this.readAttentionSnapshot(
        project.name,
        'GitLab merge-request',
        () => readGitLabMergeRequestMonitorSnapshot(owner.id),
      );
      const pipeline = this.readAttentionSnapshot(
        project.name,
        'GitLab pipeline',
        () => readGitLabPipelineMonitorSnapshot(owner.id),
      );
      const ci = this.readAttentionSnapshot(
        project.name,
        'build and quality',
        () => readCiMonitorSnapshot(owner.id),
      );
      const ciTargets = configuredCiPollingTargets(this.state.state, owner);
      const gitLabRead = this.readAttentionSnapshot(
        project.name,
        'GitLab read health',
        () => readGitLabMergeRequestReadStatus(owner.id),
      );
      return {
        projectName: project.name,
        projectPath: project.path,
        project: activeAttentionProject({
          projectName: project.name,
          projectPath: project.path,
          ...(project.displayName && project.displayName !== project.name
            ...
            : {}),
          owner,
          events: events.filter(event => event.sessionId === owner.id),
          mergeRequest: mergeRequest.value,
          pipeline: pipeline.value,
          ci: ci.value,
          gitLabRead: gitLabRead.value,
          ...(ciTargets.sonar ? { sonarTarget: ciTargets.sonar } : {}),
          snapshotError: mergeRequest.failed || pipeline.failed || ci.failed || gitLabRead.failed,
        }),
      };
    });
  }

  private readAttentionSnapshot<Value>(
    projectName: string,
    label: string,
    read: () => Value | null,
  ): { value: Value | null; failed: boolean } {
    try {
      return { value: read(), failed: false };
    } catch (error: unknown) {
      this.log(
        `Could not read current ${label} status for ${projectName}.`,
        boundedOperationFailure(error, 'Current provider status is unavailable.').display,
      );
  ...
  private async launchClaudeSession(input: {
    ...
    try {
      await this.suggestClaudeSessionTracking();
      ...
      const managedIdentity: ManagedClaudeTerminalIdentity = {
        workSessionId: session.id,
        terminalBindingId: `terminal-${crypto.randomUUID()}`,
        claudeSessionId: crypto.randomUUID(),
        editorSessionId: this.currentEditorSessionId(),
      };
      const terminalLayout = this.claudeTerminalLayout();
      const launched = launchClaudeTerminal(vscode.window, {
        ...launch.request,
        claudeSessionId: managedIdentity.claudeSessionId,
      }, {
      ...
      session = await this.attachTerminal(session, launched.terminal, {
        bindingId: managedIdentity.terminalBindingId,
        claudeSessionId: managedIdentity.claudeSessionId,
        claudeLaunchSessionId: managedIdentity.claudeSessionId,
        editorSessionId: managedIdentity.editorSessionId,
        claudePermissionMode: launched.configuration.permissionMode,
        ...(launched.configuration.cwd ? { cwd: launched.configuration.cwd } : {}),
      });
      ...
      appendMonitorEvent({
        ...
        metadata: {
          ...
          exactClaudeResumeAvailable: true,
        },
      });
      this.refreshTerminalFirstViews();
  ...
  private currentEditorSessionId(): string {
    return normalizeEditorSessionId(vscode.env.sessionId);
  }

  /**
   * VS Code keeps terminal objects across Reload Window, but only revives their
   * shells after a full editor restart. The editor session id distinguishes
   * those cases without reading terminal content or starting a process.
   */
  private restoreManagedClaudeTerminal(terminal: vscode.Terminal): void {
    if (this.closedTerminals.has(terminal) || terminal.exitStatus !== undefined) { return; }
    const identity = managedClaudeTerminalIdentity(terminal);
    if (!identity) { return; }
    let session: WorkSessionRecord | null;
    try {
      session = readWorkSession(identity.workSessionId);
    } catch {
      return;
    }
    if (!session || session.status !== 'active') { return; }
    const binding = session.terminals.find(candidate =>
      candidate.id === identity.terminalBindingId
      && managedClaudeBindingAcceptsLaunchSession(candidate, identity.claudeSessionId));
    if (!binding || binding.editorSessionId !== this.currentEditorSessionId()) { return; }
    this.operatorTerminals.attach(terminal, {
      sessionId: session.id,
      bindingId: binding.id,
    });
    this.launchedClaudeTerminals.add(terminal);
    this.sessionTree.refresh();
  }

  /** Reads only private hook event files; terminal input, output, and scrollback are never inspected. */
  private reconcileClaudeSessionTracking(): void {
    let changed = false;
    const sessions = new Map<string, WorkSessionRecord | null>();
    const liveBindings = this.operatorTerminals.listBindings();
    const liveKeys = new Set(liveBindings.map(live => `${live.sessionId}:${live.bindingId}`));
    for (const failureKey of this.claudeSessionTrackingReadFailures) {
      if (!liveKeys.has(failureKey)) { this.claudeSessionTrackingReadFailures.delete(failureKey); }
    }
    for (const live of liveBindings) {
      const failureKey = `${live.sessionId}:${live.bindingId}`;
      try {
        let session = sessions.get(live.sessionId);
        if (session === undefined) {
          session = readWorkSession(live.sessionId);
          sessions.set(live.sessionId, session);
        }
        if (!session || session.status !== 'active') { continue; }
        const binding = session.terminals.find(candidate => candidate.id === live.bindingId);
        if (!binding?.claudeSessionId || !binding.editorSessionId || !binding.claudePermissionMode) { continue; }
        const event = readClaudeSessionTrackingEvent(session.id, binding.id);
        if (!event) {
          this.claudeSessionTrackingReadFailures.delete(failureKey);
          continue;
        }
        const launchSessionId = binding.claudeLaunchSessionId || binding.claudeSessionId;
        if (event.launchClaudeSessionId !== launchSessionId
          || (binding.claudeIdentityObservedAt
            && event.observedAt <= binding.claudeIdentityObservedAt)) {
          continue;
        }
        const claudeSessionChanged = event.claudeSessionId !== binding.claudeSessionId;
        const resumeFolderChanged = event.cwd !== binding.cwd;
        const updated = updateWorkSessionClaudeIdentity(session.id, {
          terminalBindingId: binding.id,
          launchClaudeSessionId: event.launchClaudeSessionId,
        ...
        this.claudeSessionTrackingReadFailures.delete(failureKey);
        changed = true;
        if (claudeSessionChanged || resumeFolderChanged) {
          appendMonitorEvent({
            sessionId: updated.id,
            type: 'decision.recorded',
            source: 'kronos',
            summary: `${workSessionEventContext(updated).label} saved Claude resume identity updated from a lifecycle event.`,
            subject: { kind: 'work-session', id: updated.id, ...workSessionTicketMetadata(updated) },
            metadata: {
              claudeLifecycleSource: event.source,
              claudeSessionChanged,
              resumeFolderChanged,
            },
          });
        }
      } catch (error: unknown) {
        if (this.claudeSessionTrackingReadFailures.has(failureKey)) { continue; }
        this.claudeSessionTrackingReadFailures.add(failureKey);
        this.log(
          'Could not apply a Claude Session tracking event.',
          boundedOperationFailure(error, 'Claude Session tracking event was ignored.').display,
        );
      }
    }
    if (changed) { this.refreshTerminalFirstViews(); }
  }

  private async attachTerminal(
    session: WorkSessionRecord,
    terminal: vscode.Terminal,
    managed?: ManagedClaudeAttachment,
    fallbackCwd?: string,
    ...
    if (managed) {
      input.bindingId = managed.bindingId;
      input.claudeSessionId = managed.claudeSessionId;
      input.claudeLaunchSessionId = managed.claudeLaunchSessionId;
      input.editorSessionId = managed.editorSessionId;
      input.claudePermissionMode = managed.claudePermissionMode;
    } else if (previous?.sessionId === session.id) {
      input.bindingId = previous.bindingId;
    }
    if (processId !== undefined) { input.processId = processId; }
    const cwd = this.terminalWorkingDirectory(terminal) || managed?.cwd || fallbackCwd;
  ...
  private promptTemplateContext(session: WorkSessionRecord): PromptTemplateContext {
    ...
    return {
      sessionTitle: workSessionDisplayName(session),
  ...
  private async choosePromptLibraryTerminal(argument: unknown): Promise<TerminalSelection | undefined> {
    ...
    const session = candidates.length === 1 ? candidates[0] : (await vscode.window.showQuickPick(candidates.map(candidate => ({
      ...
      detail: workSessionDisplayName(candidate),
      session: candidate,
  ...
  private async createLocalHandoff(argument: unknown): Promise<void> {
    ...
    const title = await vscode.window.showInputBox({
      ...
      value: `${workSessionDisplayName(session)} handoff`,
  ...
  private async insertCiContext(
    argument: unknown,
    selectedSonarTarget?: { projectKey: string; branch: string },
  ): Promise<void> {
    const requestedProject = projectTargetStringProperty(argument, 'projectName');
    const requestedSessionId = stringProperty(argument, 'workSessionId') || stringProperty(argument, 'sessionId');
    let project: RegisteredProjectCommandTarget | undefined;
    ...
    if (requestedProject) {
      project = this.resolveRegisteredProject(argument);
      if (!project) { return; }
      selection = await this.chooseProjectInsertionTerminal(
        project,
      ...
      if (!ticketKey || !ticket) { return; }
      selection = await this.chooseInsertionTerminal(ticketKey, requestedSessionId);
      if (!selection) { return; }
    ...
    let sonarTarget = selectedSonarTarget
      || savedTargets.sonar
  ...
  private showProviderNotice(notice: ManagedProviderNotice): void {
    const projectState = this.activeAttentionStateForNotice(notice);
    const readyKey = `${notice.session.id}:ready-for-review`;
    if (projectState !== 'ready-for-review') {
      this.announcedReadyForReview.delete(readyKey);
    }
    const notificationKind = activeAttentionNotificationKind(notice.event, projectState);
    if (!notificationKind) { return; }
    if (notificationKind === 'ready-for-review' && this.announcedReadyForReview.has(readyKey)) { return; }
    if (notificationKind === 'problem') {
      this.announcedReadyForReview.delete(readyKey);
    } else {
      this.announcedReadyForReview.add(readyKey);
    }
    const providerHeadline = attentionEventHeadline(notice.event);
    const projectLabel = notice.session.projectName
      ? this.state.state?.projects[notice.session.projectName]?.display_name || notice.session.projectName
      : undefined;
    const headline = notificationKind === 'ready-for-review'
      ? `${projectLabel || 'This project'} is ready for review. All configured checks are passing and approval is still needed.`
      : providerHeadline;
    try {
      const notificationEvent: Parameters<typeof appendMonitorEvent>[0] = {
        ...
        source: 'kronos',
        summary: headline,
        metadata: { transitionEventId: notice.event.id, notificationKind },
      };
    ...
    const prompt = notificationKind === 'problem'
  ...
  private activeAttentionStateForNotice(notice: ManagedProviderNotice): ActiveAttentionProjectState | undefined {
    if (!notice.session.projectName) { return undefined; }
    return this.loadActiveAttentionProjects()
      .find(result => result.projectName === notice.session.projectName)
      ?.project?.state;
  }
  ...
  private async focusWorkSessionTerminal(argument: unknown): Promise<void> {
    ...
    if (!selected) {
      const resumableBinding = await this.chooseResumableClaudeBinding(session);
      if (resumableBinding) {
        const restoredTerminal = await this.chooseManagedClaudeTerminal(session.id, resumableBinding);
        if (restoredTerminal
          && resumableBinding.editorSessionId === this.currentEditorSessionId()) {
          session = this.prepareSessionForTerminalConnection(session);
          await this.attachTerminal(session, restoredTerminal, this.managedClaudeAttachment(resumableBinding));
        } else {
          const resumed = await this.resumeExactClaudeSession(session, resumableBinding, restoredTerminal);
          if (!resumed) { return; }
        }
      } else {
        const choice = await this.chooseLegacySessionOpenAction(session);
        if (!choice) { return; }
        if (choice.kind === 'terminal') {
          session = this.prepareSessionForTerminalConnection(session);
          await this.attachTerminal(session, choice.terminal);
        } else {
          const resumed = await this.resumeLegacyClaudeSession(session);
          if (!resumed) { return; }
        }
      }
  ...
  private prepareSessionForTerminalConnection(session: WorkSessionRecord): WorkSessionRecord {
    let prepared = session.status === 'closed' ? reopenWorkSession(session.id) : session;
    if (prepared.kind === 'ticket') {
      const ticket = this.state.state?.tickets[prepared.ticketKey];
      if (ticket) { prepared = this.ensureProviderBindings(prepared, ticket); }
    }
    return prepared;
  }

  private async chooseResumableClaudeBinding(
    session: WorkSessionRecord,
  ): Promise<ResumableWorkSessionTerminalBinding | undefined> {
    const bindings = resumableWorkSessionTerminalBindings(session);
    if (bindings.length <= 1) { return bindings[0]; }
    const pick = await vscode.window.showQuickPick(bindings.map(binding => ({
      label: binding.name,
      description: binding.status === 'closed' ? 'terminal closed' : 'resume available',
      detail: binding.cwd || session.projectPath || 'Saved working folder unavailable',
      binding,
    })), {
      title: `Resume ${workSessionEventContext(session).label}`,
      placeHolder: 'Choose the exact Claude conversation to reopen',
    });
    return pick?.binding;
  }

  private async chooseManagedClaudeTerminal(
    sessionId: string,
    binding: ResumableWorkSessionTerminalBinding,
  ): Promise<vscode.Terminal | undefined> {
    const terminals = vscode.window.terminals.filter(terminal => {
      if (this.closedTerminals.has(terminal) || terminal.exitStatus !== undefined) { return false; }
      const identity = managedClaudeTerminalIdentity(terminal);
      if (!identity
        || identity.workSessionId !== sessionId
        || identity.terminalBindingId !== binding.id
        || !managedClaudeBindingAcceptsLaunchSession(binding, identity.claudeSessionId)) {
        return false;
      }
      const liveBinding = this.operatorTerminals.bindingForTerminal(terminal);
      return !liveBinding
        || (liveBinding.sessionId === sessionId && liveBinding.bindingId === binding.id);
    });
    if (terminals.length <= 1) { return terminals[0]; }
    const pick = await vscode.window.showQuickPick(terminals.map((terminal, index) => ({
      label: terminal.name,
      description: terminal === vscode.window.activeTerminal
        ? 'focused restored terminal'
        : `restored terminal ${index + 1}`,
      terminal,
    })), {
      title: `Choose restored terminal for ${binding.name}`,
      placeHolder: 'Multiple terminals carry the same saved identity; choose explicitly',
    });
    return pick?.terminal;
  }

  private managedClaudeAttachment(
    binding: ResumableWorkSessionTerminalBinding,
    cwd = binding.cwd,
    launchClaudeSessionId = binding.claudeLaunchSessionId || binding.claudeSessionId,
  ): ManagedClaudeAttachment {
    return {
      bindingId: binding.id,
      claudeSessionId: binding.claudeSessionId,
      claudeLaunchSessionId: launchClaudeSessionId,
      editorSessionId: this.currentEditorSessionId(),
      claudePermissionMode: binding.claudePermissionMode,
      ...(cwd ? { cwd } : {}),
    };
  }

  private async resumeExactClaudeSession(
    session: WorkSessionRecord,
    binding: ResumableWorkSessionTerminalBinding,
    existingTerminal?: vscode.Terminal,
  ): Promise<vscode.Terminal | undefined> {
    const resumeKey = `${session.id}:${binding.id}`;
    if (this.claudeResumesInFlight.has(resumeKey)) {
      void vscode.window.showInformationMessage('That Claude conversation is already being resumed.');
      return undefined;
    }
    this.claudeResumesInFlight.add(resumeKey);
    let commandSubmitted = false;
    try {
      if (!this.canLaunchClaude()) { return undefined; }
      if (!await this.confirmClaudePermissionMode(binding.claudePermissionMode)) { return undefined; }
      const preparedSession = this.prepareSessionForTerminalConnection(session);
      const configuration = vscode.workspace.getConfiguration('kronos');
      const cwd = binding.cwd
        || preparedSession.projectPath
        || vscode.workspace.workspaceFolders?.[0]?.uri.fsPath
        || os.homedir();
      const identity: ManagedClaudeTerminalIdentity = {
        workSessionId: preparedSession.id,
        terminalBindingId: binding.id,
        claudeSessionId: binding.claudeSessionId,
        editorSessionId: this.currentEditorSessionId(),
      };
      const options: Parameters<typeof resumeClaudeTerminal>[2] = {
        identity,
        ...(existingTerminal
          ? { existingTerminal }
          : { location: this.claudeTerminalLaunchLocation(this.claudeTerminalLayout()) }),
      };
      const resumed = resumeClaudeTerminal(vscode.window, {
        command: configuration.get<string>('claudeCommand', DEFAULT_CLAUDE_COMMAND),
        name: binding.name,
        cwd,
        permissionMode: binding.claudePermissionMode,
        claudeSessionId: binding.claudeSessionId,
      }, options);
      commandSubmitted = true;
      this.launchedClaudeTerminals.add(resumed.terminal);
      const launchClaudeSessionId = existingTerminal
        ? managedClaudeTerminalIdentity(existingTerminal)?.claudeSessionId || binding.claudeSessionId
        : binding.claudeSessionId;
      await this.attachTerminal(
        preparedSession,
        resumed.terminal,
        this.managedClaudeAttachment(binding, cwd, launchClaudeSessionId),
      );
      appendMonitorEvent({
        sessionId: preparedSession.id,
        type: 'decision.recorded',
        source: 'operator',
        summary: `${workSessionEventContext(preparedSession).label} exact Claude conversation resumed by explicit operator action.`,
        subject: { kind: 'work-session', id: preparedSession.id, ...workSessionTicketMetadata(preparedSession) },
        metadata: {
          explicitResume: true,
          exactClaudeSession: true,
          reusedRestoredTerminal: Boolean(existingTerminal),
          claudePermissionMode: binding.claudePermissionMode,
        },
      });
      this.refreshTerminalFirstViews();
      void vscode.window.showInformationMessage(`Resumed ${binding.name} in its saved folder.`);
      return resumed.terminal;
    } catch (error: unknown) {
      if (!commandSubmitted && session.status === 'closed') {
        try { closeWorkSession(session.id); }
        catch (compensationError: unknown) {
          this.log(
            'Could not restore stopped Session state after a failed Claude resume.',
            boundedOperationFailure(compensationError, 'Session compensation failed.').display,
          );
        }
      }
      const detail = boundedOperationFailure(
        error,
        commandSubmitted
          ? 'The Claude resume command was submitted, but Kronos could not reconnect its Session.'
          : 'Claude conversation resume failed.',
      ).display;
      this.log('Could not resume the managed Claude conversation.', detail);
      void vscode.window.showErrorMessage(detail);
      return undefined;
    } finally {
      this.claudeResumesInFlight.delete(resumeKey);
    }
  }

  private async resumeLegacyClaudeSession(session: WorkSessionRecord): Promise<vscode.Terminal | undefined> {
    const resumeKey = `${session.id}:legacy-picker`;
    if (this.claudeResumesInFlight.has(resumeKey)) {
      void vscode.window.showInformationMessage('Claude’s conversation picker is already opening for this Session.');
      return undefined;
    }
    this.claudeResumesInFlight.add(resumeKey);
    let commandSubmitted = false;
    try {
      ...
      if (!await this.confirmClaudePermissionMode(launch.validated.permissionMode)) { return undefined; }
      const preparedSession = this.prepareSessionForTerminalConnection(session);
      const savedCwd = [...preparedSession.terminals]
        .sort((left, right) => right.attachedAt.localeCompare(left.attachedAt))
        .find(binding => binding.cwd)?.cwd;
      const cwd = savedCwd || preparedSession.projectPath || launch.validated.cwd;
      const request = {
        ...launch.request,
        ...(cwd ? { cwd } : {}),
        selectSession: true,
      };
      const resumed = resumeClaudeTerminal(vscode.window, request, {
        location: this.claudeTerminalLaunchLocation(this.claudeTerminalLayout()),
      });
      commandSubmitted = true;
      this.launchedClaudeTerminals.add(resumed.terminal);
      await this.attachTerminal(preparedSession, resumed.terminal, undefined, resumed.configuration.cwd);
      appendMonitorEvent({
        sessionId: preparedSession.id,
        type: 'decision.recorded',
        source: 'operator',
        summary: `${workSessionEventContext(preparedSession).label} opened Claude's conversation picker by explicit operator action.`,
        subject: { kind: 'work-session', id: preparedSession.id, ...workSessionTicketMetadata(preparedSession) },
        metadata: {
          explicitResume: true,
          exactClaudeSession: false,
          legacySessionPicker: true,
          claudePermissionMode: resumed.configuration.permissionMode,
        },
      });
      this.refreshTerminalFirstViews();
      void vscode.window.showInformationMessage('Claude’s conversation picker is open. Choose the conversation for this legacy Session.');
      return resumed.terminal;
    } catch (error: unknown) {
      if (!commandSubmitted && session.status === 'closed') {
        try { closeWorkSession(session.id); }
          ...
          this.log(
            'Could not restore stopped Session state after a failed legacy resume.',
            boundedOperationFailure(compensationError, 'Session compensation failed.').display,
          );
        }
      }
      const detail = boundedOperationFailure(
        error,
        commandSubmitted
          ? 'Claude’s conversation picker opened, but Kronos could not reconnect its Session.'
          : 'Claude’s conversation picker could not be opened.',
      ).display;
      this.log('Could not open Claude’s conversation picker.', detail);
      void vscode.window.showErrorMessage(detail);
      return undefined;
    } finally {
      this.claudeResumesInFlight.delete(resumeKey);
  ...
  private async chooseLegacySessionOpenAction(
    session: WorkSessionRecord,
  ): Promise<LegacySessionOpenChoice | undefined> {
    const terminals = vscode.window.terminals.filter(terminal => {
      const binding = this.operatorTerminals.bindingForTerminal(terminal);
      return (!binding || binding.sessionId === session.id)
        && !managedClaudeTerminalIdentity(terminal);
    });
    const pick = await vscode.window.showQuickPick([
      {
        label: '$(history) Resume a Claude conversation…',
        description: 'Recommended for a Session created before exact resume support',
        detail: 'Opens Claude’s own conversation picker in the Session’s saved folder.',
        choice: { kind: 'claude-picker' } as LegacySessionOpenChoice,
      },
      ...terminals.map((terminal, index) => ({
        label: terminal.name,
        description: terminal === vscode.window.activeTerminal ? 'focused terminal' : `open terminal ${index + 1}`,
        detail: 'Connect this already-running terminal without submitting a command.',
        choice: { kind: 'terminal', terminal } as LegacySessionOpenChoice,
      })),
    ], {
      title: `Open ${workSessionEventContext(session).label}`,
      placeHolder: 'Choose how to reconnect this legacy Session',
    });
    return pick?.choice;
  }
  ...
  private async renameManagedSession(argument: unknown): Promise<void> {
    const session = await this.resolveWorkSession(argument, true);
    if (!session) { return; }
    const previousName = workSessionDisplayName(session);
    const value = await vscode.window.showInputBox({
      title: `Rename ${previousName}`,
      prompt: 'Optional local display name. Leave blank to restore the original name. Project, Jira, Claude resume, terminal, and provider identities stay unchanged.',
      value: session.displayName || '',
      placeHolder: session.title,
      validateInput: input => input.trim().length > 300 ? 'Session display name must be 300 characters or fewer.' : undefined,
    });
    if (value === undefined) { return; }
    if (value.trim().length > 300) {
      void vscode.window.showErrorMessage('Session display name must be 300 characters or fewer.');
      return;
    }
    try {
      const updated = setWorkSessionDisplayName(session.id, value);
      const nextName = workSessionDisplayName(updated);
      appendMonitorEvent({
        sessionId: session.id,
        type: 'decision.recorded',
        source: 'operator',
        summary: updated.displayName
          ? 'Session display name changed locally by the operator.'
          : 'Session display name restored to its original local title.',
        subject: { kind: 'work-session', id: session.id, ...workSessionTicketMetadata(session) },
        metadata: { displayNameChanged: true, displayNameCleared: !updated.displayName },
      });
      this.refreshTerminalFirstViews();
      void vscode.window.showInformationMessage(updated.displayName
        ? `Session renamed to ${nextName}. Its terminal, Claude conversation, project, Jira contexts, and provider links were not changed.`
        : `Session name restored to ${nextName}. Its terminal and durable identities were not changed.`);
    } catch (error: unknown) {
      const detail = boundedOperationFailure(error, 'Kronos could not update the Session display name.').display;
      this.log('Could not update Session display name.', detail);
      void vscode.window.showErrorMessage(detail);
    }
  }
  ...
  private async chooseProjectInsertionTerminal(
    ...
    let session = requestedSessionId
      ...
      : activeBinding ? sessions.find(candidate => candidate.id === activeBinding.sessionId) : undefined;
    if (requestedSessionId && !session) {
      void vscode.window.showWarningMessage(
        `The selected Session is no longer active for ${project.displayName || project.projectName}. Choose a current Session and try again.`,
      );
      return undefined;
    }
    ...
    const selected = await this.chooseLiveTerminal(session.id);
    if (!selected) {
      void vscode.window.showWarningMessage(`Focus or reconnect the terminal for ${workSessionDisplayName(session)} before adding context.`);
      return undefined;
    }
  ...
  private async acknowledgeAttention(argument: unknown): Promise<void> {
    ...
    const liveRow = this.currentLiveAttentionRow(eventId, sessionId);
    if (liveRow && !liveRow.dismissible) {
      void vscode.window.showInformationMessage(
        'This provider check is still running or pending. It will become clearable after a completed result is available.',
      );
      return;
    }
    try {
      if (liveRow) {
        acknowledgePresentedMonitorEvent(liveRow.event);
      } else {
      ...
      const detail = boundedOperationFailure(
        error,
        'This Attention item changed before it could be cleared. Refresh Attention and try again.',
      ).display;
      this.log('Attention item could not be cleared.', detail);
      void vscode.window.showWarningMessage(detail);
      return;
    }
  ...
  private currentLiveAttentionRow(eventId: string, sessionId: string): ActiveAttentionRow | undefined {
    for (const result of this.loadActiveAttentionProjects()) {
      const row = result.project?.rows.find(candidate =>
        candidate.event.id === eventId && candidate.event.sessionId === sessionId
      );
      if (row) { return row; }
    }
    return undefined;
  ...
  private async openProvider(argument: unknown): Promise<void> {
    ...
    if (stringProperty(argument, 'source') === 'sonar') {
      await this.selectMonitoredSonarBranch(argument, providerUrl);
    }
  }

  private async selectMonitoredSonarBranch(argument: unknown, providerUrl: string): Promise<void> {
    let projectKey: string | undefined;
    let branch: string | undefined;
    try {
      const url = new URL(providerUrl);
      projectKey = url.searchParams.get('id')?.trim();
      branch = url.searchParams.get('branch')?.trim();
    } catch (error: unknown) {
      this.reportSonarBranchSelectionFailure(
        boundedOperationFailure(error, 'Kronos could not use the selected SonarQube branch because its provider link is invalid.').display,
      );
      return;
    }
    if (!projectKey || !branch) {
      this.reportSonarBranchSelectionFailure(
        'Kronos could not use the selected SonarQube branch because its provider link is missing a project or branch.',
      );
      return;
    }
    ...
    if (!sessionId) {
      this.reportSonarBranchSelectionFailure(
        'Kronos could not use the selected SonarQube branch because the Attention item is not linked to a current Session. Refresh Attention and try again.',
      );
      return;
    }
    let workSession: WorkSessionRecord | null = null;
    let session: WorkSessionRecord | null = null;
    try {
      workSession = readWorkSession(sessionId);
      session = workSession || this.readProjectMonitorById(sessionId);
    } catch (error: unknown) {
      this.reportSonarBranchSelectionFailure(
        boundedOperationFailure(error, 'Kronos could not read the Session for the selected SonarQube branch. Refresh Attention and try again.').display,
      );
      return;
    }
    if (!session) {
      this.reportSonarBranchSelectionFailure(
        'Kronos could not use the selected SonarQube branch because its Session is no longer available. Refresh Attention and try again.',
      );
      return;
    }
    const projectName = session.projectName;
    if (!projectName) {
      this.reportSonarBranchSelectionFailure(
        'Kronos could not use the selected SonarQube branch because its Session is not linked to a registered project.',
      );
      return;
    }
    const project = this.state.state?.projects[projectName];
    if (!project) {
      this.reportSonarBranchSelectionFailure(
        `${projectName} is no longer registered in Kronos. Register the project again, refresh Attention, and retry the branch.`,
      );
      return;
    }
    if (project.config.sonar_project_key === projectKey && project.config.sonar_branch === branch) { return; }
    try {
      this.state.setLocalProjectSonarTarget(projectName, projectKey, branch);
    } catch (error: unknown) {
      this.reportSonarBranchSelectionFailure(
        boundedOperationFailure(error, 'Kronos could not save the selected SonarQube monitoring branch.').display,
      );
      return;
    }
    try {
      ...
      const detail = boundedOperationFailure(
        error,
        `${projectName} now monitors SonarQube branch ${branch}, but Kronos could not record that decision in local history.`,
      ).display;
      this.log('Could not record the selected SonarQube branch decision.', detail);
      void vscode.window.showWarningMessage(detail);
    }
    this.refreshTerminalFirstViews();
    void vscode.window.showInformationMessage(
      `${projectName} will now monitor SonarQube branch ${branch}. Preparing fresh build & quality context in a new review.`,
    );

    const projectPath = project.path || session.projectPath;
    const refreshArgument = workSession?.status === 'active'
      && workSession.kind === 'ticket'
      && this.state.state?.tickets[workSession.ticketKey]
      ? { ticketKey: workSession.ticketKey, workSessionId: workSession.id }
      : projectPath
        ? {
            projectName,
            projectPath,
            ...(workSession?.status === 'active' ? { workSessionId: workSession.id } : {}),
          }
        : undefined;
    if (!refreshArgument) {
      const detail = `${projectName} now monitors SonarQube branch ${branch}, but its registered project folder is unavailable, so Kronos could not prepare fresh build & quality context.`;
      this.log('Fresh CI context preparation after SonarQube branch selection was skipped.', detail);
      void vscode.window.showWarningMessage(detail);
      void this.pollProviders(false);
      return;
    }
    try {
      await this.insertCiContext(refreshArgument, { projectKey, branch });
    } catch (error: unknown) {
      const detail = boundedOperationFailure(
        error,
        `${projectName} now monitors SonarQube branch ${branch}, but fresh build & quality context could not be prepared.`,
      ).display;
      this.log('Fresh CI context preparation after SonarQube branch selection failed.', detail);
      void vscode.window.showWarningMessage(detail);
    } finally {
      void this.pollProviders(false);
    }
  ...
  private reportSonarBranchSelectionFailure(detail: string): void {
    this.log('SonarQube branch selection was not applied.', detail);
    void vscode.window.showWarningMessage(detail);
  }

  private async suggestClaudeSessionTracking(): Promise<void> {
    if (this.claudeSessionTrackingSuggestionShown) { return; }
    const status = claudeSessionTrackingStatus();
    if (status.state === 'ready' || status.state === 'blocked') { return; }
    this.claudeSessionTrackingSuggestionShown = true;
    const choice = await vscode.window.showInformationMessage(
      'Keep this Kronos Session current if Claude changes conversation with /resume, /clear, or a manual claude --resume?',
      status.actionLabel,
      'Not Now',
    );
    if (choice === status.actionLabel) { await this.configureClaudeSessionTracking(); }
  }

  private async configureClaudeSessionTracking(): Promise<void> {
    const status = claudeSessionTrackingStatus();
    if (status.state === 'ready') {
      void vscode.window.showInformationMessage(status.detail);
      return;
    }
    if (status.state === 'blocked') {
      void vscode.window.showWarningMessage(status.detail);
      return;
    }
    const action = status.actionLabel;
    const choice = await vscode.window.showInformationMessage(
      `${action} for Kronos-managed Claude terminals?`,
      {
        modal: true,
        detail: 'Kronos will add one local SessionStart command hook to your user Claude settings and keep a private backup. The hook receives Claude lifecycle metadata and updates only the matching Session conversation UUID and saved resume folder. It never reads terminal content or the Claude transcript.',
      },
      action,
    );
    if (choice !== action) { return; }
    let result: ReturnType<typeof installClaudeSessionTracking>;
    try {
      result = installClaudeSessionTracking();
    } catch (error: unknown) {
      const detail = boundedOperationFailure(
        error,
        'Claude Session tracking setup did not complete.',
      ).display;
      this.log('Claude Session tracking setup failed.', detail);
      void vscode.window.showErrorMessage(
        `${detail} Review Setup when convenient; Kronos Sessions remain available without automatic conversation tracking.`,
      );
      return;
    }
    this.claudeSessionTrackingSuggestionShown = true;
    ...
    this.renderOperationsPanels();
    this.log(
      'Claude Session tracking enabled.',
      result.backupPath
        ? 'Claude settings were backed up before the Kronos SessionStart hook was installed.'
        : 'The Kronos SessionStart hook was installed in new Claude settings.',
    );
    void vscode.window.showInformationMessage(
      'Claude Session tracking is enabled. Kronos will keep the conversation UUID and saved resume folder current for managed terminals.',
    );
  }
  ...
  private async executeOperationsPanelAction(source: 'setup' | 'doctor', action: string): Promise<void> {
    ...
    try {
      if (action === 'refreshPanel') {
        ...
        await vscode.commands.executeCommand('workbench.action.openSettings', '@ext:jmacke01.kronos claude');
      } else if (action === 'configureClaudeSessionTracking') {
        await this.configureClaudeSessionTracking();
  ...
  private operationsReadiness(): OperationsReadinessItem[] {
    ...
    const claudeTracking = claudeSessionTrackingStatus();
    ...
    return buildOperationsReadiness({
      ...
      claudeSessionTracking: {
        status: claudeTracking.state === 'ready'
          ? 'pass'
          : claudeTracking.state === 'blocked' ? 'fail' : 'warn',
        detail: claudeTracking.detail,
        actionLabel: claudeTracking.actionLabel,
      },
      providerEnvironment: {
