...
export type WorkSessionClaudePermissionMode =
  | 'default'
  | 'acceptEdits'
  | 'plan'
  | 'auto'
...
export interface WorkSessionTerminalBinding {
  ...
  /** Claude UUID fixed in this terminal's creation environment. */
  claudeLaunchSessionId?: string;
  /** Newest lifecycle event applied to the current Claude UUID and resume folder. */
  claudeIdentityObservedAt?: string;
...
export interface ResumableWorkSessionTerminalBinding extends WorkSessionTerminalBinding {
  claudeSessionId: string;
  editorSessionId: string;
  claudePermissionMode: WorkSessionClaudePermissionMode;
}
...
interface WorkSessionRecordBase {
  ...
  /** Optional presentation-only override. Stable identity and launch metadata remain unchanged. */
  displayName?: string;
  title: string;
...
export interface UpdateWorkSessionClaudeIdentityInput {
...
export function workSessionEventContext(record: WorkSessionRecord): WorkSessionEventContext {
  const displayName = workSessionDisplayName(record);
  const context: WorkSessionEventContext = {
    sessionId: record.id,
    sessionTitle: displayName,
    label: record.projectName
      ? `${record.projectName}: ${displayName}`
      : record.kind === 'ticket' ? `${record.ticketKey}: ${displayName}` : displayName,
  };
  if (record.kind === 'ticket') { context.ticketKey = record.ticketKey; }
  return context;
}

/** Returns the operator-facing Session name without changing its durable title or identity. */
export function workSessionDisplayName(record: Pick<WorkSessionRecord, 'displayName' | 'title'>): string {
  return record.displayName || record.title;
}
...
/** Returns only bindings that can target one exact Claude conversation. */
export function resumableWorkSessionTerminalBindings(
  session: WorkSessionRecord,
): ResumableWorkSessionTerminalBinding[] {
  return session.terminals
    .filter((binding): binding is ResumableWorkSessionTerminalBinding =>
      Boolean(binding.claudeSessionId && binding.editorSessionId && binding.claudePermissionMode))
    .sort((left, right) =>
      (right.detachedAt || right.attachedAt).localeCompare(left.detachedAt || left.attachedAt)
...
export function attachWorkSessionTerminal(
  ...
  return mutateWorkSession(sessionId, options, (record, at) => {
    ...
    const claudeSessionId = optionalClaudeSessionId(input.claudeSessionId);
    const claudeLaunchSessionId = optionalClaudeSessionId(input.claudeLaunchSessionId);
    const editorSessionId = optionalEditorSessionId(input.editorSessionId);
    const claudePermissionMode = optionalClaudePermissionMode(input.claudePermissionMode);
    ...
    const existing = existingIndex >= 0 ? record.terminals[existingIndex] : undefined;
    const retainedClaudeSessionId = claudeSessionId || existing?.claudeSessionId;
    const retainedClaudeLaunchSessionId = claudeLaunchSessionId
      || existing?.claudeLaunchSessionId
      || retainedClaudeSessionId;
    const retainedEditorSessionId = editorSessionId || existing?.editorSessionId;
    const retainedClaudePermissionMode = claudePermissionMode || existing?.claudePermissionMode;
    if (retainedClaudeSessionId) { candidate.claudeSessionId = retainedClaudeSessionId; }
    if (retainedClaudeLaunchSessionId) { candidate.claudeLaunchSessionId = retainedClaudeLaunchSessionId; }
    if (existing?.claudeIdentityObservedAt
      && retainedClaudeLaunchSessionId === (existing.claudeLaunchSessionId || existing.claudeSessionId)) {
      candidate.claudeIdentityObservedAt = existing.claudeIdentityObservedAt;
    }
    if (retainedEditorSessionId) { candidate.editorSessionId = retainedEditorSessionId; }
    if (retainedClaudePermissionMode) { candidate.claudePermissionMode = retainedClaudePermissionMode; }
    if (existingIndex >= 0) {
...
/**
 * Applies one correlated Claude SessionStart event without changing Session
 * project, Jira contexts, providers, display name, or terminal ownership.
 */
export function updateWorkSessionClaudeIdentity(
  sessionId: string,
  input: UpdateWorkSessionClaudeIdentityInput,
  options: WorkSessionStoreOptions = {},
): WorkSessionRecord {
  return mutateWorkSession(sessionId, options, record => {
    requireActiveSession(record);
    const bindingId = normalizeEntityId(input.terminalBindingId, 'terminal binding id');
    const binding = record.terminals.find(candidate => candidate.id === bindingId);
    if (!binding) { throw new Error(`Terminal binding not found: ${bindingId}`); }
    if (!binding.claudeSessionId || !binding.editorSessionId || !binding.claudePermissionMode) {
      throw new Error('Terminal binding is not a managed Claude terminal.');
    }
    const launchClaudeSessionId = normalizeClaudeSessionId(input.launchClaudeSessionId);
    const expectedLaunchSessionId = binding.claudeLaunchSessionId || binding.claudeSessionId;
    if (launchClaudeSessionId !== expectedLaunchSessionId) {
      throw new Error('Claude lifecycle event does not match the managed terminal launch identity.');
    }
    const observedAt = normalizeTimestamp(input.observedAt, 'Claude identity observedAt');
    if (binding.claudeIdentityObservedAt
      && observedAt <= binding.claudeIdentityObservedAt) {
      return;
    }
    binding.claudeLaunchSessionId = launchClaudeSessionId;
    binding.claudeSessionId = normalizeClaudeSessionId(input.claudeSessionId);
    binding.cwd = requiredAbsolutePath(input.cwd, 'terminal cwd');
    binding.claudeIdentityObservedAt = observedAt;
  });
}

/** Matches creation metadata to either the current or launch-time Claude UUID. */
export function managedClaudeBindingAcceptsLaunchSession(
  binding: Pick<WorkSessionTerminalBinding, 'claudeSessionId' | 'claudeLaunchSessionId'>,
  claudeSessionIdValue: unknown,
): boolean {
  let candidate: string;
  try { candidate = normalizeClaudeSessionId(claudeSessionIdValue); }
  catch { return false; }
  return candidate === binding.claudeSessionId || candidate === binding.claudeLaunchSessionId;
}
...
/** Changes only the local Session display override; blank restores the generated title. */
export function setWorkSessionDisplayName(
  sessionId: string,
  displayNameValue: string | undefined,
  options: WorkSessionStoreOptions = {},
): WorkSessionRecord {
  const candidate = displayNameValue?.trim();
  if (candidate && candidate.length > 300) {
    throw new Error('Work session display name must be 300 characters or fewer.');
  }
  return mutateWorkSession(sessionId, options, record => {
    const displayName = optionalSingleLine(displayNameValue, 'work session display name', 300);
    if (displayName && displayName !== record.title) {
...
export function normalizeWorkSessionRecord(
  ...
  const displayName = optionalSingleLine(value['displayName'], 'work session display name', 300);
  if (displayName && displayName !== base.title) { base.displayName = displayName; }
...
function normalizeTerminalBinding(value: unknown): WorkSessionTerminalBinding {
  ...
  const claudeSessionId = optionalClaudeSessionId(value['claudeSessionId']);
  const claudeLaunchSessionId = optionalClaudeSessionId(value['claudeLaunchSessionId']);
  const claudeIdentityObservedAt = optionalTimestamp(
    value['claudeIdentityObservedAt'],
    'terminal Claude identity observedAt',
  );
  const editorSessionId = optionalEditorSessionId(value['editorSessionId']);
  const claudePermissionMode = optionalClaudePermissionMode(value['claudePermissionMode']);
  const managedIdentityFieldCount = [
    claudeSessionId,
    editorSessionId,
    claudePermissionMode,
  ].filter(Boolean).length;
  if (managedIdentityFieldCount !== 0 && managedIdentityFieldCount !== 3) {
    throw new Error('Terminal managed Claude identity must include session id, editor session id, and permission mode together.');
  }
  if ((claudeLaunchSessionId || claudeIdentityObservedAt) && managedIdentityFieldCount !== 3) {
    throw new Error('Terminal Claude lifecycle identity requires a complete managed Claude identity.');
  }
  if (claudeIdentityObservedAt && !claudeLaunchSessionId) {
    throw new Error('Terminal Claude lifecycle timestamp requires a launch session id.');
  }
  ...
  if (claudeSessionId) { binding.claudeSessionId = claudeSessionId; }
  if (claudeLaunchSessionId) { binding.claudeLaunchSessionId = claudeLaunchSessionId; }
  if (claudeIdentityObservedAt) { binding.claudeIdentityObservedAt = claudeIdentityObservedAt; }
  if (editorSessionId) { binding.editorSessionId = editorSessionId; }
  if (claudePermissionMode) { binding.claudePermissionMode = claudePermissionMode; }
...
function optionalClaudeSessionId(value: unknown): string | undefined {
  if (value === undefined || value === null || value === '') { return undefined; }
  return normalizeClaudeSessionId(value);
}

function optionalEditorSessionId(value: unknown): string | undefined {
  if (value === undefined || value === null || value === '') { return undefined; }
  return normalizeEditorSessionId(value);
}

function optionalClaudePermissionMode(value: unknown): WorkSessionClaudePermissionMode | undefined {
  if (value === undefined || value === null || value === '') { return undefined; }
  if (value === 'default'
    || value === 'acceptEdits'
    || value === 'plan'
  ...
  throw new Error('Terminal Claude permission mode is invalid.');
}
