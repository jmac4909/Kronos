...
export function setLocalProjectIntegrations(
  ...
  for (const value of values.slice(0, MAX_LOCAL_PROJECTS)) {
    ...
    const jenkinsUrl = normalizeOptionalJenkinsJobUrl(value.jenkinsUrl, `${name} Jenkins job URL`);
...
export function parseProjectBranchProfiles(value: string): ProjectBranchProfile[] {
  ...
  for (const [index, rawLine] of value.replace(/\r\n?/g, '\n').split('\n').entries()) {
    ...
    seen.add(branch);
    const jenkinsUrl = normalizeOptionalJenkinsJobUrl(parts[1], `${branch} Jenkins job URL`);
...
function normalizeOptionalJenkinsJobUrl(value: unknown, label: string): string | undefined {
  const candidate = safeSingleLine(value, 4_000);
  if (!candidate) { return undefined; }
  const normalized = normalizeProjectJenkinsUrl(candidate);
  if (!normalized) {
    throw new Error(
      `${label} must be a full HTTPS Jenkins /job/<name>/ URL (or loopback HTTP URL) without credentials, `
      + 'a view/console/API suffix, or a numbered build.',
    );
  }
  return normalized;
}
