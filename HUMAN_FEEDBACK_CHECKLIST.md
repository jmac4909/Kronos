...
2. Confirm provider failures, recoveries, partial reads, and monitoring blockers are grouped by registered project when known, with real Jira context retained, rather than scattered by provider. Confirm GitLab MR rows use an orange pull-request icon, Jenkins rows use a red server-process icon, and SonarQube rows use a blue shield icon. Confirm each row also writes out project, provider, subject, severity, observed time, last changed time, and why it needs attention so meaning never depends on color alone.
