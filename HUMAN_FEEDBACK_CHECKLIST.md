...
3. Confirm newly registered projects open **Project Integration Setup**. Give one project a nickname, enter a GitLab numeric ID or `group/project` path, Jenkins job URL, SonarQube project key, and default branch; confirm credential readiness is shown without any secret values. Leave an optional field blank and confirm it can be cleared.
...
- Before linking any Jira ticket or opening any terminal Session, use **Poll Managed Providers**. Confirm the configured project shows an active/latest poll in Projects, GitLab/Jenkins/SonarQube reads occur for that project, and resulting Attention items are grouped under the project. Open an MR for the current branch, poll, then switch the same repository to a second ticket branch and open another MR without linking either Jira ticket; confirm the next poll discovers the second IID and gives it its own project-owned Attention row. Reload VS Code and confirm polling resumes from the same private project owner without creating a Session row or fake Jira identity.
...
13. Choose the inline or expanded **Start Claude in project** action. Confirm exactly one terminal opens in that registered directory, its title shows the current branch, and the new Session has the project identity but no Jira ticket or context.
14. Use **Set project nickname**, **Insert MR evidence**, and the other Project-row actions on a registered Project after refreshing or changing its discovery-root path spelling. Confirm every action re-resolves the registered project instead of showing **Select a currently registered project**. With a project-only Session focused, confirm MR and Jenkins/Sonar evidence open the editable composer and insert without adding Jira context. Confirm the nickname appears in Projects, Work, the Jira board, project setup, local search, Sessions, and Attention while its stable identity, canonical path, linked tickets/sessions, provider configuration, and Git state remain unchanged. Clear it and confirm every surface falls back to the stable project name.
15. Insert the project's `[GIT-project]` context into its explicitly attached session. Confirm the composer previews changed paths, potential credential material is redacted from the private artifact, and the terminal line is not submitted.
16. Use **Open merge request page**. Confirm an existing MR opens when known; otherwise a prefilled GitLab new-MR browser page opens and no MR is created until you act in GitLab.
17. In **Project Integration Setup**, add two explicit branch-profile lines using `branch | Jenkins URL | SonarQube key | SonarQube branch`, choose one optional active fallback, save, and reopen the form. Confirm both profiles and the active choice round-trip exactly.
18. Link a ticket whose known MR source branch exactly matches the second profile, poll once, and confirm Jenkins uses that profile's job/branch while SonarQube uses its project key/provider branch. Use a ticket without a matching MR profile and confirm only the explicit active fallback is used.
19. Enter a duplicate branch, unsafe branch, credential-bearing URL, or active name not present in the profile list. Confirm save is refused with a bounded repair message and the prior valid setup remains unchanged.
...
9. Run **Poll Managed Providers** once and confirm the Project, not the Session, receives the latest provider health.
...
11. Observe Jenkins results `ABORTED`, `CANCELED`/`CANCELLED`, and `UNSTABLE`, plus a SonarQube warning gate, using safe fixtures. Confirm each is warning/failure state on first observation and on later transitions, and that a later successful result is shown as recovered.
